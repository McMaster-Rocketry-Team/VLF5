        ...
        loop {
            ...
            packet_builder.update(|packet| {
                ...
                if status_stream_stale(&payload_status_receipt) {
                    ...
                    // being asserted, and the packet's own liveness bits are
                    // what tell the operator why the flags went quiet.
                    packet.sem_experiment_flags = Default::default();
                }
        ...
        loop {
            ...
            packet_builder.update(|packet| match message {
                ...
                CanBusMessageEnum::CustomPayloadStatus(message) => {
                    ...
                    // handler's.
                    packet.sem_experiment_flags = message.experiment_flags();
                }
                _ => {}
            });
        }
