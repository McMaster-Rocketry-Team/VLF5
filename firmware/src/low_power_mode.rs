...
async fn low_power_mode(
    ...
    let wait_low_power_mode_end_fut = async {
        ...
        match receiver.try_get().unwrap() {
            AvionicsMode::Armed => todo!(),
            AvionicsMode::Landed => todo!(),
            AvionicsMode::SelfTest => todo!(),
            AvionicsMode::LowPower => unreachable!(),
        }
    };
